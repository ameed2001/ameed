...
const Navbar = () => {
  ...
  return (
    <nav className="bg-slate-800 text-white shadow-nav">
      <div className="container mx-auto flex justify-center items-center">
        <ul className="flex flex-wrap justify-center items-center space-x-1 rtl:space-x-reverse">
          {navLinks.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className={cn(
                  "flex items-center gap-2 px-3 py-3 text-sm font-semibold transition-colors duration-200 hover:bg-slate-700",
